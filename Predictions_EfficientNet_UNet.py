...
# Used to copy model from dbfs to databricks tmp folder
#dbutils.fs.cp("dbfs:/mnt/mydata/BraTS/efficientb0_unet.hdf5", "file:/tmp/efficientb0_unet.hdf5")


# Load model from trained_models folder
model = keras.models.load_model("trained_models/efficientb0_unet.hdf5",
                                compile = False)
...
# Use model to predict 6 random images and display the images
...
def showPredictsById_t1ce(case, start_slice = 60):
    ...
    for i in range(6): 
        axarr[i].imshow(cv2.resize(origImage[:,:,start_slice+VOLUME_START_AT], (128, 128)), cmap="gray", interpolation='none')
    
    axarr[0].imshow(cv2.resize(origImage[:,:,start_slice+VOLUME_START_AT], (128, 128)), cmap="gray")
    axarr[0].title.set_text('Original T1ce image')
    curr_ground_truth=cv2.resize(ground_truth[:,:,start_slice+VOLUME_START_AT], (128, 128), interpolation = cv2.INTER_NEAREST)
...
# Use model to predict on 6 random images and display the images
