...
#dbutils.fs.cp("dbfs:/mnt/mydata/BraTS/resnet50_unet.hdf5", "file:/tmp/resnet50_unet.hdf5")


# load trained model
model = keras.models.load_model("trained_models/resnet50_unet.hdf5",
                                 compile = False)
...
def showPredictsById_t2(case, start_slice = 60):
    # Path needs to be change to local directory
    path = f"/dbfs/mnt/mydata/BraTS/MICCAI_BraTS2020_TrainingData/BraTS20_Training_{case}"
    ground_truth = nib.load(os.path.join(path, f'BraTS20_Training_{case}_seg.nii')).get_fdata()
